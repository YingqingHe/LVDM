...
class BasicTransformerBlockST(nn.Module):
    ...
    def _forward(self, x, context=None, mask=None,):
        ...
        for i in range(0, 64, 16):
            x_ = x[:, i:i+16, :]
            if i != 0:
                x3 = self.attn1_tmp(self.norm4(x_), context=x_prev, mask=mask) + x_
            else:
                x3 = self.attn1_tmp(self.norm4(x_), context=None, mask=mask) + x_
            xs.append(x3)
            x_prev = x_
        ...
        for i in range(0, 64, 16):
            x_ = x[:, i:i+16, :]
            if i != 0:
                x2 = self.attn2_tmp(self.norm5(x_), context=x_prev, mask=mask) + x_
            else:
                x2 = self.attn2_tmp(self.norm5(x_), context=None, mask=mask) + x_
